...
vic = 5;                        % this is the vicinity neighbourhood in units to be checked
...
while (flag==1)
    ...
    if (ch ~= 0)  % If not lying on any of the edges, then proceed further
     qbestNeighb = MinCostNode(G,qnewtemp,vic,start, qnear);   % calculate the best node in the vicinity
     check = CheckIntersection(qbestNeighb,qnewtemp,edges);    % 1 means no intersection
    ...
    if (check==1)
        ...
        if (findedge(G,PointToNode(qbestNeighb),PointToNode(qnewtemp)))==0  % to check if edge exists in the graph
            G = addedge(G,PointToNode(qbestNeighb), PointToNode(qnewtemp));            
        end
